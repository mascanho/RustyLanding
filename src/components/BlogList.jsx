          ...
          <div className="container relative">
            <div className="text-center mb-16">
              ...
              <p className="body-1 max-w-2xl mx-auto text-n-3 mb-12 leading-relaxed">
                Discover cutting-edge insights, comprehensive tutorials, and the latest updates from the RustySEO team.
                Stay ahead in the world of SEO, web development, and digital marketing.
              </p>
              ...
              <div className="flex flex-col sm:flex-row gap-4 max-w-xl mx-auto mb-8">
                <div className="relative flex-1">
                  ...
                  />
                  <svg className="w-5 h-5 text-n-3 absolute left-4 top-1/2 -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="relative bg-n-8/30 backdrop-blur-sm border border-n-6/30 rounded-3xl p-8 lg:p-12">
                    <div className="flex items-center gap-3 mb-6">
                      <div className="px-4 py-2 bg-gradient-to-r from-purple-500/20 to-blue-500/20 border border-purple-500/30 rounded-full">
                        <span className="text-purple-300 text-sm font-semibold">✨ Featured</span>
                      </div>
                          ...
                          <div>
                            <div className="flex items-center gap-3 mb-4">
                              <div className="w-12 h-12 bg-gradient-to-r from-purple-500 to-blue-500 rounded-full flex items-center justify-center">
                                <span className="text-white font-bold">
                                  {featuredPost.author.split(' ').map(name => name[0]).join('').slice(0, 2).toUpperCase()}
                                </span>
                              </div>
                              <div>
                                <div className="text-n-1 font-semibold">{featuredPost.author}</div>
                            ...
                            <div className="flex flex-wrap gap-2 mb-6">
                              {featuredPost.tags.map((tag) => (
                                <span key={tag} className="px-3 py-1 bg-n-7/60 text-n-2 rounded-full text-xs">
                                  {tag}
                                </span>
                              ))}
                            </div>
                          ...
                          <div className="hidden md:block">
                            <div className="aspect-video bg-gradient-to-br from-purple-500/20 to-blue-500/20 rounded-2xl flex items-center justify-center">
                              <div className="text-center">
                                ...
                                <p className="text-n-3 text-sm">Featured Content</p>
                              </div>
                            </div>
                          </div>
                        </div>
            ...
            <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-8">
              {filteredPosts.length > 0 ? filteredPosts.map((post) => (
                ...
                >
                  ...
                  {post.excerpt && (
                    <p className="body-2 text-n-3 mb-6 line-clamp-3 leading-relaxed">{post.excerpt}</p>
                  )}

                  <div className="flex items-center justify-between">
                    <div className="flex flex-wrap gap-2">
                      {post.tags?.slice(0, 2).map((tag) => (
                        <span key={tag} className="px-3 py-1 bg-n-7/60 text-n-2 rounded-full text-xs font-medium">
                          {tag}
                        </span>
                      ))}
                    </div>

                    <Link to={`/blog/${post.slug}`} className="text-color-1 hover:text-color-2 font-code text-sm transition-colors group/link">
                      Read more
                ...
                <div className="col-span-full text-center py-16">
                  <div className="w-16 h-16 bg-n-8/60 rounded-full flex items-center justify-center mb-4 mx-auto">
                    <svg className="w-8 h-8 text-n-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                  </div>
                  <h3 className="h4 text-n-1 mb-2">No Articles Found</h3>
                  <p className="body-1 text-n-3">Try adjusting your search or filter criteria.</p>
                </div>
              )}
            </div>
            ...
            <div className="mt-20 text-center">
              <div className="bg-gradient-to-r from-purple-500/10 via-blue-500/5 to-purple-500/10 border border-purple-500/20 rounded-3xl p-8 lg:p-12 max-w-4xl mx-auto">
                ...
                <p className="body-1 text-n-3 mb-8 max-w-2xl mx-auto">
                  ...
                  Join our community of developers and marketers.
                </p>
